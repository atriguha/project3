...
# filelist=st.file_uploader("Upload your files here",accept_multiple_files=True)
# # print(filelist.name)
...
tab1, tab2,tab3 = st.tabs(["Constant Inputs", "Filtering Technique Inputs","Download Converted Files"])

with tab1:
    st.header("Constant Inputs")
...
def useful_values():
    ...
    data.at[0, 'anisotropy'] = round(
        data.at[0, 'std_w\'']/data.at[0, 'std_u\''], 3)
...
def fk():
    ...
    data.at[index, 'fku_2d'] = round(
        (data.at[0, 'u\'u\'u\' mean']+data.at[0, 'u\'w\'w\' mean'])*constant_fk2d, 3)
    data.at[index, 'Fku_2d'] = round(
        data.at[index, 'fku_2d']/Shear_velocity, 3)

    data.at[index, 'fkw_2d'] = round(
        (data.at[0, 'w\'w\'w\' mean']+data.at[0, 'w\'u\'u\' mean'])*constant_fk2d, 3)
    data.at[index, 'Fkw_2d'] = round(
        data.at[index, 'fkw_2d']/Shear_velocity, 3)

    data['fku_3d'] = data['Fku_3d'] = data['fkw_3d'] = data['Fkw_3d'] = data['TKE_3d'] = ''

    data.at[index, 'fku_3d'] = round((data.at[0, 'u\'u\'u\' mean'] + data.at[0,
                                     'u\'w\'w\' mean'] + data.at[0, 'u\'v\'v\' mean'])*multiplying_factor_3d, 3)
    data.at[index, 'Fku_3d'] = round(
        data.at[index, 'fku_2d']/Shear_velocity, 3)

    data.at[index, 'fkw_3d'] = round((data.at[0, 'w\'w\'w\' mean']+data.at[0,
                                     'w\'u\'u\' mean'] + data.at[0, 'w\'v\'v\' mean'])*multiplying_factor_3d, 3)
    data.at[index, 'Fkw_3d'] = round(
        data.at[index, 'fkw_3d']/Shear_velocity, 3)

    data.at[index, 'TKE_3D'] = round(
        (data.at[0, 'Var_v\'']*data.at[0, 'Var_u\'']*data.at[0, 'Var_w\'']) * multiplying_factor_3d, 3)
...
def octant_column():
    ...
    data.at[index, 'Min_Octant_Count'] = data.at[index,
                                                 'Min_Octant_Count_id'] = N
    data.at[index, 'Max_Octant_Count'] = data.at[index,
                                                 'Max_Octant_Count_id'] = 0
...
def octant():
    ...
    data.at[index, 'Probability_Octant_plus_1'] = round(
        octant_values_frequency[1+4]/N, 3)
    ...
    data.at[index, 'Probability_Octant_plus_2'] = round(
        octant_values_frequency[2+4]/N, 3)
    ...
    data.at[index, 'Probability_Octant_plus_3'] = round(
        octant_values_frequency[3+4]/N, 3)
    ...
    data.at[index, 'Probability_Octant_plus_4'] = round(
        octant_values_frequency[4+4]/N, 3)
...
def Acceleration_U():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, 'accl_U'] = (
                data.at[i, 'U']-data.at[i-1, 'U'])/data.at[1, 'Time']
...
def Acceleration_V():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, 'accl_V'] = data.at[i, 'V'] - \
                data.at[i-1, 'V']/data.at[1, 'Time']
...
def Acceleration_W():
    for i in range(N):
        if i == 0:
            ...
            data.at[i, 'accl_W'] = data.at[i, 'W'] - \
                data.at[i-1, 'W']/data.at[1, 'Time']
...
def replace_all_4(i):
    if i <= 12 or i > N-12:
        ...
        x2 = i+1
        y2 = i+13
        data.at[i, 'U'] = (data['U'].iloc[x1:y1].mean() +
                           data['U'].iloc[x2:y2].mean())/2
        data.at[i, 'V'] = (data['V'].iloc[x1:y1].mean() +
                           data['V'].iloc[x2:y2].mean())/2
        data.at[i, 'W'] = (data['W'].iloc[x1:y1].mean() +
                           data['W'].iloc[x2:y2].mean())/2
...
def add_front_name(name, x):
    if (x == 1):
        name = name + "_previous_point"
    if (x == 2):
        name = name + "_2*last-2nd_last"
    if (x == 3):
        name = name + "_overall_mean"
    if (x == 4):
        name = name + "_12_point_strategy"
    if (x == 5):
        name = name + "_mean_of_previous_2point"
...
# tch = (input('Chose Filtering Method From Above:'))
# import matplotlib.pyplot as pltAC
with tab2:
    st.header("Filtering Technique Inputs")


    tchmap={'C':1,'S':2,'A':3,'C & S':4,'C & A':5,'S & A':6,'C & S & A':7,'All combine':8}
    option = st.selectbox(
        'Choose Filtering Technique',
        ('C', 'S', 'A', 'C & S','C & A', 'S & A','C & S & A', 'All combine'))
...
#          '6. S & A\n', '7. C & S & A\n', '8. all combine\n')
    ...
    print(tch)
    if tch == 1:
        corr = int(float(st.number_input('Enter thresold value C:')))
    elif tch == 2:
        SNR = int(float(st.number_input('Enter thresold value S:')))
    elif tch == 3:
    ...
    schmap={'previous point':1,'2*last-2nd_last':2,'overall_mean':3,'12_point_strategy':4,'mean of previous 2 point'
    :5,'all seqential':6,'all parallel':7}
    option = st.selectbox(
        'Choose Filtering Technique',
        ('previous point', '2*last-2nd_last', 'overall_mean', '12_point_strategy','mean of previous 2 point', 
        'all seqential','all parallel'))
    ...
    if sch > 7:
        ...
        filelist = st.sidebar.file_uploader(
            "Upload your files here", accept_multiple_files=True)
        
        butt = st.button("Convert")
    # print(filelist.name)
            # fileList = open('input_file_list.txt', 'r')
